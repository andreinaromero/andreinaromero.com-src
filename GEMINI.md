...
2. **Separadores:** Enmarcar el bloque del video entre dos líneas separadoras `___`. **IMPORTANTE:** Debe haber una línea en blanco antes y después de cada `___` y del código `iframe` para asegurar que se rendericen correctamente como líneas horizontales.
3. **Texto Introductorio:** Incluir la frase `Puedes ver el procedimiento completo aquí:` seguido de una línea en blanco.
